...
-- Fix terminal mouse popups:
-- 1. Disables right-click context popups globally (keeps split resizing working)
-- vim.opt.mousemodel = "extend"

-- 2. Swallows right-clicks in Terminal Mode so Neovim doesn't open menus when focus is in the terminal
-- vim.keymap.set("t", "<RightMouse>", "<NOP>", { noremap = true, silent = true })
...
-- <leader> key + v + t
-- This will open a new vertical terminal on the right that has no line numbers,
-- and you will be insert mode when this terminal opens up.
vim.keymap.set(
    "n", "<leader>vt",
    ":belowright vsplit<CR>"
    .. ":vertical resize 34<CR>"
    .. ":terminal<CR>"
    .. ":setlocal nonumber norelativenumber<CR>"
    .. "i",
    { desc = "[v]ertical [t]erminal", noremap = true, silent = true }
)
...
-- SECTION: Plugin Keymaps => nvim-tree.lua

vim.keymap.set(
    "n",
    "<leader>ft",
    ":NvimTreeToggle<CR>",
    { desc = "[f]file [t]ree", noremap = true, silent = true }
)
